...
// delay when polling device
const WITH_DEVICE_POLLING_DELAY = 500;
...
let errorRemapping = (e: any) => throwError(() => e);
...
const identifyTransport = (t: any) => (typeof t.id === 'string' ? t.id : '');

const needsCleanup: Record<string, boolean> = {};
// When a series of APDUs are interrupted, this is called
// so we don't forget to cleanup on the next withDevice
export const cancelDeviceAction = (transport: TransportBLE): void => {
...
export class DeviceQueuedJobsManager {
  ...
  getLastQueuedJob(deviceId: string): QueuedJob {
    return (
      this.queuedJobsByDevice[deviceId] || {job: Promise.resolve(), id: -1}
    );
  }
...
export const withDevice =
  ...
  <T>(job: (t: TransportBLE) => Observable<T>): Observable<T> =>
    new Observable(o => {
      ...
      let resolveQueuedJob: (value: PromiseLike<void> | void) => void;
      ...
      const tracer = new LocalTracer(LOG_TYPE, {
        jobId,
        deviceId,
        origin: 'haqq-hw:withDevice',
      });
      tracer.trace(`New job for device: ${deviceId || 'unknown BLE device'}`);
      ...
      const finalize = async (
        ...
        try {
          ...
          tracer.trace(
            `An error occurred when closing transport (ignoring it): ${error}`,
            {
              error,
              function: 'finalize',
            },
      ...
      previousQueuedJob.job
        .then(() => {
          tracer.trace(
            'Previous queued job resolved, now trying to get a Transport instance',
            {
              previousJobId: previousQueuedJob.id,
              currentJobId: jobId,
            },
        ...
        .then(transport => {
          ...
          sub = job(transport)
            .pipe(
              catchError(error =>
                initialErrorRemapping(error, tracer.getContext()),
              ),
...
export const retryWhileErrors = (acceptError: (arg0: Error) => boolean) => {
  return (attempts: Observable<any>): Observable<any> => {
    return attempts.pipe(
      mergeMap(error => {
        ...
        return timer(WITH_DEVICE_POLLING_DELAY);
      }),
    );
  };
};
...
export const withDevicePolling =
  (deviceId: string) =>
  <T>(
    job: (arg0: TransportBLE) => Observable<T>,
